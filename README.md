...
**What pitwall is not: a guaranteed cost or quality optimizer.** In our own A/B runs on well-specified single-repo builds, the orchestration layer lost on speed and quality against doing the task solo in one session. Cost is where measurement pays: most lanes ranged from a clear loss to a tie, but one out of eleven — glm-5.2 on codex — came in separably cheaper than not orchestrating, and hit 0.32x the solo cost on one run. Coordination output, lane latency, and multi-lane authorship are overhead the task must be large enough to absorb. Use pitwall to find and verify the pairing that wins on your own work, not because delegation is presumed cheaper or better. See [Findings](#findings) for the runs that measured it.
