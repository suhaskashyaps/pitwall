...
byte-identical seed, against a solo control. This is the run behind the
...
The control is a plain Claude Code session on the same seed with no
orchestration.
...
| leg | lane $ | architect $ | total $ | total/solo | lane/solo | wall clock | tests |
|---|---|---|---|---|---|---|---|
| solo control | n/a | n/a | 1.129865 | 1.000x | n/a | 4m02s | 97 |
| `glm-codex` | 0.478210 | 0.860098 | 1.338308 | 1.184x | 0.423x | 9m15s | 72 |
| `codex-gpt56` | 0.650406 | 0.775725 | 1.426131 | 1.262x | 0.576x | 5m10s | 72 |
| `grok45-codex` | 0.471628 | 1.043434 | 1.515061 | 1.341x | 0.417x | 6m50s | 72 |
| `sonnet5-grok` | 0.665705 | 0.980427 | 1.646132 | 1.457x | 0.589x | 8m25s | 81 |
| `kimi-cc` | 0.833693 | 0.878077 | 1.711770 | 1.515x | 0.738x | 9m01s | 73 |
| `glm-grok` | 0.170299 | 1.874153 | 2.044452 | 1.809x | 0.151x | 22m41s | 97 |
| `grok` | 0.372563 | 1.799875 | 2.172439 | 1.923x | 0.330x | 8m41s | 72 |
| `glm-cc` | 1.470253 | 0.890597 | 2.360850 | 2.089x | 1.301x | 8m34s | 72 |
| `gpt56-grok` | 1.209069 | 1.374873 | 2.583942 | 2.287x | 1.070x | 12m02s | 72 |
| `opus48-grok` | 1.253729 | 1.530256 | 2.783984 | 2.464x | 1.110x | 8m13s | 72 |
| `claude-native` | 4.472366 | 0.831390 | 5.303756 | 4.694x | 3.958x | 8m27s | 82 |
...
orchestrating. The cheapest total was 1.18x the control. This is the fourth
...
completely, and six lanes beat the control on `lane $`. None beat it on total.
`glm-grok` is the clean demonstration: the cheapest implementation in the matrix
at $0.17, or 0.151x the control, still finished at 1.81x once its architect was
...
implementation tied the control on test count.
...
Then run one control leg with `--skip-pitwall` under its own experiment id.
